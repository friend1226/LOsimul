...
class Rhea(Character):
    ...
    def _active1(self,
        ...
        for t in targets:
            if targets[t] > 0:
                t.give_buff(BT.ELEMENT_RES[E.FIRE], 0, bv[0], _round=2, efft=BET.DEBUFF, max_stack=1,
                            tag=G.FLOOD_FIRE, desc=G.FLOOD)
                t.give_buff(BT.ELEMENT_RES[E.ICE], 0, -bv[0], _round=2, efft=BET.DEBUFF, max_stack=1,
                            tag=G.FLOOD_ICE, desc=G.FLOOD)
                t.give_buff(BT.ELEMENT_RES[E.ELEC], 0, -bv[0], _round=2, efft=BET.DEBUFF, max_stack=1,
                ...
                if targets[t] > 1:
                    t.give_buff(BT.ACC, 0, -bv1, _round=2, efft=BET.DEBUFF, max_stack=1, tag="Rhea_A1_ACC", desc=desc)
                    t.give_buff(BT.EVA, 0, -bv1, _round=2, efft=BET.DEBUFF, max_stack=1, tag="Rhea_A1_EVA", desc=desc)
        return {t: (self.calc_damage(t, atk_rate[t], element=element, wr=wr) if targets[t] > 0 else 0) for t in targets}
    ...
    def _active2(self,
        ...
        for t in targets:
            if targets[t] > 0:
                t.give_buff(BT.DEF, 1, bv[0], _round=2, efft=BET.DEBUFF, desc=desc)
                t.give_buff(BT.TAKEDMGINC, 1, bv[1], _round=2, efft=BET.DEBUFF, desc=desc)
    ...
    def _passive1(self, tt: str, args: Any, targets: List[Tuple[int, int]], bv: List[NUM_T]):
        if tt == TR.ROUND_START and self.find_buff(type_=BT.SPD, efft=BET.BUFF):
            ...
            self.give_buff(BT.CRIT, 0, bv[0]*200/3, _round=1, efft=BET.BUFF, desc=desc)
            self.give_buff(BT.ACC, 0, bv[0]*200, _round=1, efft=BET.BUFF, desc=desc)

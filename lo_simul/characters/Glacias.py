...
class Glacias(Character):
    ...
    def _active2(self,
        ...
        for t in targets:
            if targets[t] > 0:
                if t.find_buff(type_=BT.ELEMENT_RES[E.ICE], efft=BET.DEBUFF):
                    t.give_buff(BT.TAKEDMGINC, 1, bv[0], data=D.DmgInfo(element=E.ICE), round_=0, desc=desc)
                if t.find_buff(tag=G.FLOOD):
    ...
    def _passive2(self, tt: str, args: Optional[Dict[str, Any]], targets: List[Tuple[int, int]], bv: List[NUM_T]):
        ...
        if tt == TR.ROUND_START:
            for t in self.get_passive_targets(targets):
                ...
                t.give_buff(BT.ELEMENT_RES[E.FIRE], 0, bv[3], round_=1, efft=BET.BUFF, desc=desc)
    ...
    def _passive3(self, tt: str, args: Optional[Dict[str, Any]], targets: List[Tuple[int, int]], bv: List[NUM_T]):
        ...
        if tt == TR.WAVE_START:
            for t in self.get_passive_targets(targets, True):
                ...
                t.give_buff(BT.ELEMENT_RES[E.ICE], 0, bv[2], round_=5, efft=BET.DEBUFF, desc=desc)

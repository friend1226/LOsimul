...
class OrangeAdeOld(Character):
    ...
    def _active1(self,
        ...
        for t in targets:
            if targets[t] > 0:
                t.give_buff(BT.INSTANT_DMG, 1, bv[0], data=D.DmgInfo(subject=self), desc=desc)
                t.give_buff(BT.IMMUNE_BUFF, 0, 1, data=D.BuffCond(efft=BET.BUFF), round_=2, desc=desc)
    ...
    def _active2(self,
        ...
        self.give_buff(BT.ATK, 0, bv[0], proportion=(self, BT.ATK), efft=BET.BUFF, round_=2, desc=desc)
        return {}
    ...
    def get_skill_cost(self, skill_no):
        __r = super().get_skill_cost(skill_no)
        if skill_no == 2:
            __r += 1
        return __r

...
class OrangeAde(Character):
    ...
    def _active2(self,
        ...
        desc = "정보 공유"
        for t in targets:
            t.give_buff(BT.ATK, 0, bv[0], proportion=(self, BT.ATK), efft=BET.BUFF, round_=3,
                        max_stack=2, tag="Orangeade_ATK", desc=desc)
        return {}
    ...
    def _passive3(self, tt: str, args: Optional[Dict[str, Any]], targets: List[Tuple[int, int]], bv: List[NUM_T]):
        ...
        if tt == TR.WAVE_START:
            allys = self.get_passive_targets(targets)
            if len([c for c in allys if c.type_[0] == CT.LIGHT]) <= 2:
                for t in allys:
                    t.give_buff(BT.RANGE, 0, 1 + (self.skillvl[4] == 10), round_=99, desc=desc)
                    t.give_buff(BT.ACC, 0, bv[0], round_=99, desc=desc)

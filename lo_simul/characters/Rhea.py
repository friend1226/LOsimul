        ...
        for t in targets:
            if targets[t] > 0:
                ...
                if t.find_buff(tag=G.CORROSION):
                    t.give_buff(BT.TAKEDMGINC, 1, bv1/100, round_=0, data=D.FDmgInfo(element=E.ELEC), desc="급속 부식")
                if targets[t] > 1:
    ...
    def _active2(self,
        ...
        for t in targets:
            if targets[t] > 0:
                ...
                t.give_buff(BT.TAKEDMGINC, 1, 1, round_=0, efft=BET.DEBUFF, data=D.FDmgInfo(element=E.FIRE), desc=desc)
                if targets[t] > 1:
